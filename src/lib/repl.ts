...
export async function prepare(code: string, language: string) {
  ...
  switch (language) {
    ...
    case 'cpp': {
      await fs.copyFile(
        path.join(LANG_PATH, language, 'cryptopp.sh'),
        path.join(rpath, 'cryptopp.sh')
      );
...
export async function run(id: string, language: string, context: any) {
  ...
  const sourceFiles = {
    ...
    cpp: ['Dockerfile', 'main.cpp', 'cryptopp.sh'],
  };
  ...
  try {
    ...
    await DockerManager.buildImage(rpath, id, buildStream, sourceFiles[language]);
  } catch (ex) {
    send({
      type: 'debug',
  ...
  try {
    ...
    const success = await DockerManager.runContainer(id, send, runStream, context);
    await sleep(1000);
